...
class PrometheusMiddleware:
    ...
    def __init__(
            self, app: ASGIApp, group_paths: bool = True, app_name: str = "teko_app",
            prefix: str = "teko_http", buckets: Optional[List[str]] = None,
            filter_unhandled_paths: bool = False, exclude_paths: Optional[List[str]] = []
    ):
        self.app = app
    ...
    def request_count(self):
        ...
        if metric_name not in PrometheusMiddleware._metrics:
            PrometheusMiddleware._metrics[metric_name] = Counter(
                metric_name,
                "Total HTTP requests",
                ("method", "url_rule", "status", "app_name"),
            )
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if self._get_router_path(scope) in self.exclude_paths:
                return
